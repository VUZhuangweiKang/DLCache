...
class _DLCJobDataLoaderIter(_BaseDataLoaderIter):
    ...
    def _next_data(self):
        ...
        if self._last_iter_time is not None:
            if len(self._req_time) > self._num_batches:
        ...
        while True:
            try:
                ...
                idx, data, _active_workers, dur  = self._get_data()
                self._tasks_outstanding -= 1
        ...
        if _active_workers is not None and self._rcvd_idx > 1:
            # buffer `num_batches` load time measurements
            if len(self._load_time_cache[_active_workers]) > self._num_batches:
                self._load_time_cache[_active_workers].pop(0)
