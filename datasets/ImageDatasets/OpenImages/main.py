...
parser = argparse.ArgumentParser(description='PyTorch OpenImange Training')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.distributed:
        val_sampler = torch.utils.data.distributed.DistributedSampler(dataset_test, shuffle=False, drop_last=True)
    else:
        val_sampler = None
    ...
    if args.evaluate:
        ...
        if args.distributed:
            ...
            train_sampler = None
            data_loader_train = DLCJobDataLoader(dataset_train, batch_size=args.batch_size, shuffle=True, num_workers=args.workers, pin_memory=True, sampler=train_sampler)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(data_loader_train, model, criterion, optimizer, epoch, args)

        # evaluate on validation set
        acc1 = validate(data_loader_test, model, criterion, args)
